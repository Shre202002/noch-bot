...
import axios from 'axios';
import * as cheerio from 'cheerio';
import { writeKnowledge, readKnowledge, findAccountById } from '@/lib/storage';
import { getUserIdFromCookie } from '@/lib/auth';
import { embedText } from '@/lib/embeddings';
import { qdrant, ensureCollection, COLLECTION } from '@/lib/qdrant';
import { checkCrawlLimit, logCrawl } from '@/lib/ratelimit';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Check crawl rate limit ────────────────────────────────
    const account = await findAccountById(userId);
    const plan = account?.plan || 'free';

    const crawlCheck = await checkCrawlLimit(userId, plan);
    if (!crawlCheck.allowed) {
      return NextResponse.json(
        {
          error: `Monthly crawl limit reached (${crawlCheck.used}/${crawlCheck.limit}). Please upgrade your plan to crawl more websites.`,
          rateLimited: true,
    ...
    if (allContent.length === 0) {
      return NextResponse.json(
        { error: 'Could not extract content from this site.' },
    ...
    // ── Log this crawl for rate limiting ─────────────────────
    await logCrawl(userId, url);
