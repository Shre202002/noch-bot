...
//       // ✅ Update UI properly
//       setCrawlLogs([`✅ Crawled successfully`])
...
//     // Add empty assistant bubble immediately (typing indicator)
//     setPreviewMessages([...newMessages, { role: 'assistant', content: '' }])
...
//       if (data.error) {
//         setPreviewMessages([...newMessages, { role: 'assistant', content: '⚠️ ' + data.error }])
//       } else {
//         setPreviewMessages([...newMessages, { role: 'assistant', content: data.text }])
//       }
//     } catch (err: any) {
...
export default function ConfigurePage() {
  ...
  const sendPreviewMessage = async (text?: string) => {
    const userText = text || previewInput.trim()
    if (!userText || previewLoading || !userId) return
                  ...
                  previewMessages.map((msg, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                      {msg.content === '' && msg.role === 'assistant' ? (
                        <div style={{ background: '#1f2228', border: '1px solid #2a2d35', borderRadius: 18, padding: '12px 16px', display: 'flex', gap: 4 }}>
